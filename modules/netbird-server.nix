{ config, lib, pkgs, nixpkgs-netbird, ... }:
let
  inherit (lib)
    filterAttrs
    literalExpression
  ...
  settingsFile = settingsFormat.generate "setup.env" (builtins.mapAttrs
    (_: val:
...
in
{
  ...
  options.services.netbird-server = {
    ...
    dashboard = mkPackageOption (nixpkgs-netbird.legacyPackages.x86_64-linux) "netbird-dashboard" { };

    settings = mkOption {
      type = with types; attrsOf (nullOr (oneOf [ (listOf str) bool int float str ]));
    ...
    enableDeviceAuthorizationFlow = mkEnableOption "device authorization flow for netbird." // { default = true; };
    ...
    enableACME = mkEnableOption "the automatic retrieval of TLS certificates." // {
      default = true;
    };

    management.dnsDomain = mkOption {
      type = types.str;
      default = "netbird.selfhosted";
      description = lib.mdDoc "Domain used for peer resolution.";
    };

    management.singleAccountModeDomain = mkOption {
      type = types.str;
      default = "netbird.selfhosted";
      description = lib.mdDoc ''
    ...
    management.disableAnonymousMetrics = mkOption {
      type = types.bool;
      default = true;
      description = lib.mdDoc "Disables push of anonymous usage metrics to NetBird.";
    };

    management.disableSingleAccountMode = mkOption {
      type = types.bool;
      default = false;
      description = lib.mdDoc ''
  ...
  config = mkMerge [
    (mkIf cfg.enable {
      ...
      services.nginx.virtualHosts = mkIf cfg.enableNginx {
        ${cfg.settings.NETBIRD_DOMAIN} = {
          ...
          locations."/api".proxyPass = "http://localhost:${builtins.toString cfg.ports.management}";
      ...
      systemd.services.netbird-setup = {
        wantedBy = [ "netbird-management.service" "netbird-signal.service" "multi-user.target" ];
        serviceConfig = {
          Type = "oneshot";
        ...
        path = (with pkgs; [ coreutils findutils gettext gnused ]) ++ (optionals cfg.setupAutoOidc (with pkgs; [ curl jq ]));
        ...
        '' + (optionalString cfg.setupAutoOidc ''
          ...
          envsubst '$NETBIRD_AUTH_AUTHORITY $NETBIRD_AUTH_JWT_CERTS $NETBIRD_AUTH_TOKEN_ENDPOINT $NETBIRD_AUTH_DEVICE_AUTH_ENDPOINT $NETBIRD_AUTH_PKCE_AUTHORIZATION_ENDPOINT $NETBIRD_AUTH_CLIENT_ID $NETBIRD_AUTHENTIK_PASSWORD' < ${stateDir}/management.json > ${stateDir}/management.json.copy
        '') + ''
          ...
          envsubst '$USE_AUTH0 $AUTH_AUTHORITY $AUTH_CLIENT_ID $AUTH_CLIENT_SECRET $AUTH_SUPPORTED_SCOPES $AUTH_AUDIENCE $NETBIRD_MGMT_API_ENDPOINT $NETBIRD_MGMT_GRPC_API_ENDPOINT $NETBIRD_HOTJAR_TRACK_ID $AUTH_REDIRECT_URI $AUTH_SILENT_REDIRECT_URI $NETBIRD_TOKEN_SOURCE $NETBIRD_DRAG_QUERY_PARAMS $NETBIRD_AUTH_CLIENT_ID' < "$MAIN_JS".copy > "$MAIN_JS"
    ...
    (mkIf cfg.enableCoturn {
      ...
      networking.firewall = {
        allowedUDPPorts = with settings; [ TURN_PORT (TURN_PORT + 1) 5349 5350 ];
        allowedTCPPorts = with settings; [ TURN_PORT (TURN_PORT + 1) ];
        allowedUDPPortRanges = [{ from = settings.TURN_MIN_PORT; to = settings.TURN_MAX_PORT; }];
      };
    })

    (mkIf cfg.enableACME {
      services.coturn = let cert = config.security.acme.certs.${settings.TURN_DOMAIN}; in {
        cert = "${cert.directory}/fullchain.pem";
        pkey = "${cert.directory}/key.pem";
      };

      users.users.nginx.extraGroups = mkIf cfg.enableCoturn [ "turnserver" ];

      # share certs with coturn and restart on renewal
      security.acme.certs.${settings.TURN_DOMAIN} = mkIf cfg.enableCoturn {
        group = "turnserver";
